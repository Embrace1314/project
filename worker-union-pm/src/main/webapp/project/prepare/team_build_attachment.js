...
function btnDeleteAttachment(obj) {
    ...
    if (!isEmpty(attachmentId)) {
        if (confirm("确定删除该附件？")) {
            // 删除附件
            ajaxRemoveProjectTeamAttachment(getUrlParam("project.id"), attachmentId, function(data) {
                // 刷新附件显示
                refreshTeamAttachments();
            }, function(data) {
            }, true, waiting, run, true);
...
var uploaderParams = {
    ...
    init : {
        ...
        FilesAdded : function(uploader, files) {
            // 开始上传
            ajaxAcquireProjectTeamAttachmentUpToken(
                getUrlParam("project.id"), function(data) {
