 ...
 * 显示技术方案文件列表
 */
function showTechnicalList() {
    ajaxListTechnicalProposalAttachment(getUrlParam("project.id"), function(data) {
        //显示最近一次上传文件表格
        ...
        //显示技术方案文件列表
        removeTableTr("#tableTechnical");
        for (var i = 0; i < data.attachments.length; i++) {
            $("#tableTechnical").append(
                ...
                +'</td><td class="list_last_col"><button class="btn_delete" type="button" onclick="btnDeleteProposal(this)">删除</button></td></tr>');
        }
        }, function(data) {
    }, true, waiting, run, true);
}
 ...
 * 删除技术方案
 */
function btnDeleteProposal(obj) {
    var attachmentId = $(obj).parent().parent().children("td").first().html();
    if (confirm("确定删除该技术方案？")) {
        ajaxRemoveTechnicalProposalAttachmentById(getUrlParam("project.id"), attachmentId, function (data) {
            //刷新技术方案列表
            showTechnicalList();
        }, function (data) {
        }, true, waiting, run, true);
...
var uploaderParams = {
    ...
    init: {
        ...
        FilesAdded: function (uploader, files) {
            // 开始上传
            ajaxAcquireTechnicalProposalUpToken(getUrlParam("project.id"), function (data) {
...
function btnDownloadFile(obj) {
    // 获取授权信息
    var attachmentId = $(obj).parent().parent().children("td").first().html();
    ajaxAcquireTechnicalProposalDownToken(getUrlParam("project.id"), attachmentId,
        function(data) {
