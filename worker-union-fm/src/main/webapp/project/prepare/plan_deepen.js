    ...
    ajaxListSchemeDeepenAttachment(getUrlParam("project.id"), function(data) {
        ...
        for (var i = 0; i < data.attachments.length; i++) {
                $("#tablePlan").append(
                    ...
                    +'</a></td><td>' + data.attachments[i].staffName
...
function btnPass(obj) {
    ...
    if(confirm("确定通过该附件的审核？")){
        ajaxPassSchemeDeepenAttachment(getUrlParam("project.id"), attachmentAudit,
            function(data) {
                //刷新列表
                showPlanList();
            }, function() {
...
function btnRebut(obj) {
    ...
    if(confirm("确定驳回该附件的审核？")){
        ajaxFailSchemeDeepenAttachment(getUrlParam("project.id"), attachmentAudit,
            function(data) {
                //刷新列表
                showPlanList();
            }, function() {
