...
function showFileList() {
    ajaxListFinalReportAttachment(getUrlParam("project.id"), function(data) {
        removeTableTr("#tableSummary");
        for (var i = 0; i < data.attachments.length; i++) {
            $("#tableSummary").append(
                ...
                +'"></td><td class="list_verify_col"><div class="div_verify_btn">'
                +'<button class="btn_pass" type="button" onclick="btnPass(this)">通过</button><div class="div_line"></div>'
                +'<button class="btn_rebut" type="button" onclick="btnRebut(this)">驳回</button></div></td><td class="list_last_col"><button class="btn_delete_disable" type="button" disabled="disabled">删除</button></td></tr>');
        }
        }, function(data) {
    }, true, waiting, run, true);
}
...
function btnPass(obj) {
    ...
    if(confirm("确定通过该附件的审核？")){
        ajaxPassFinalReportAttachment(getUrlParam("project.id"), attachmentAudit,
            function(data) {
                //刷新列表
                showFileList();
            }, function() {
...
function btnRebut(obj) {
    ...
    if(confirm("确定驳回该附件的审核？")){
        ajaxFailFinalReportAttachment(getUrlParam("project.id"), attachmentAudit,
            function(data) {
                //刷新列表
                showFileList();
            }, function() {
...
function btnDownloadFile(obj) {
    ...
    ajaxAcquireFinalReportDownToken(getUrlParam("project.id"), attachmentId,
        function(data) {
