 ...
 * @param isLoadingExplicit
 */
function ajaxPassCompletionDataAttachment(projectId, attachmentAudit, success,
	...
	$.ajax({
		url : URL_PASS_COMPLETION_DATA_ATTACHMENT,
		data : {
			"project.id" : projectId,
 ...
 * @param isLoadingExplicit
 */
function ajaxFailCompletionDataAttachment(projectId, attachmentAudit, success,
		fail, isExplicit, waiting, run, isLoadingExplicit) {
	// 显示缓冲
	if (isLoadingExplicit) {
		waiting();
	...
	$.ajax({
		url : URL_FAIL_COMPLETION_DATA_ATTACHMENT,
		data : {
			"project.id" : projectId,
			"attachmentAudit.attachmentId" : attachmentAudit.attachmentId,
			"attachmentAudit.opinion" : attachmentAudit.opinion
