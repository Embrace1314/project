 ...
 */
function ajaxGetLatestSettlementAttachment(projectId, success, fail,
 ...
 *            缓冲是否显示处理
 */
function ajaxPageSettlementItem(ProjectId, page, success, fail, isExplicit,
 ...
 * @param isLoadingExplicit
 */
function ajaxGetSettlementItemById(projectId, settlementItemId, success, fail,
		isExplicit, waiting, run, isLoadingExplicit) {
	// 显示缓冲
	if (isLoadingExplicit) {
		waiting();
 ...
 *            缓冲是否显示处理
 */
function ajaxPageSettlementAttachment(ProjectId, page, success, fail,
		isExplicit, waiting, run, isLoadingExplicit) {
	// 显示缓冲
	if (isLoadingExplicit) {
		waiting();
 ...
 * @param isLoadingExplicit
 */
function ajaxGetSettlementItemDetailById(projectId, settlementItemId, success,
	...
	$.ajax({
		url : URL_GET_SETTLEMENT_ITEM_DETAIL_BY_ID,
		data : {
			"project.id" : projectId,
			"settlementItem.id" : settlementItemId
		},
 ...
 * @param isLoadingExplicit
 */
function ajaxPassSettlementAttachment(projectId, attachmentAudit, success,
		fail, isExplicit, waiting, run, isLoadingExplicit) {
	// 显示缓冲
	if (isLoadingExplicit) {
		waiting();
	...
	$.ajax({
		url : URL_PASS_SETTLEMENT_ATTACHMENT,
		data : {
			"project.id" : projectId,
 ...
 * @param isLoadingExplicit
 */
function ajaxFailSettlementAttachment(projectId, attachmentAudit, success,
		fail, isExplicit, waiting, run, isLoadingExplicit) {
	// 显示缓冲
	if (isLoadingExplicit) {
		waiting();
	...
	$.ajax({
		url : URL_FAIL_SETTLEMENT_ATTACHMENT,
		data : {
			"project.id" : projectId,
			"attachmentAudit.attachmentId" : attachmentAudit.attachmentId,
			"attachmentAudit.opinion" : attachmentAudit.opinion
