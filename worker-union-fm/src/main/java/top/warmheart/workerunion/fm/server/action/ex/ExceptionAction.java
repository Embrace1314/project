...
public class ExceptionAction extends ActionJson {
	...
	private JSONObject parse(Exception ex) {
		...
		if (ex instanceof WhInvalidCertificateAttachmentException) {
			jresult.put(WebErrorEnum.CODE, WebErrorEnum.INVALID_CERTIFICATE_ATTACHMENT.errorCode);
			jresult.put(WebErrorEnum.CODE_DESC, WebErrorEnum.INVALID_CERTIFICATE_ATTACHMENT.errorCodeDesc);
			return jresult;
		}
		/*
		 * 已删除异常
		...
		if (ex instanceof WhCertificateAttachmentDeletedException) {
			jresult.put(WebErrorEnum.CODE, WebErrorEnum.CERTIFICATE_ATTACHMENT_DELETED.errorCode);
			jresult.put(WebErrorEnum.CODE_DESC, WebErrorEnum.CERTIFICATE_ATTACHMENT_DELETED.errorCodeDesc);
			return jresult;
		}
